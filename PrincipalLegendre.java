...
public class PrincipalLegendre {
	public static int p;
	public static int d;
	public static int n;
    ...
    {
		///////////////////////////////////////////
		conj=false; //use conjecture to evaluate sign of func. eq.
		ecf=true; //with or without elliptic curve
		p=11; //base prime
		d=2; //degree of twists on F_p^n
		ell=5; //order of characters
		//////////////////////////////////////////
		q=p;
		n=1;
		...
		if(!conj) N=n*d+1; //compute monics up to degree N
		else N=n*d;
			...
			while(f)
			{
				...
				if(amcompnb>1)
				{
					...
					for(k=0;k<amcompnb;k++)
					{
						...
						if(amflag && (amcomp[0][k]==0 || amcomp[0][k]==p-1 || (monic[1][k]+1)%n!=0)) amflag=false;
					}
		...
		for(z=0;z<fsize;z++)
		{
			...
			while(g)
			{
				...
				for(i=0;i<=N;i++) for(j=0;j<=ell-1;j++) { coef[i][j]=0; coef2[i][j]=0;}
				coef[0][0]=1;
				coef2[0][0]=1;
				for(i=0;i<=N+1;i++) pt[i]=0;
				
					
				if(deg%2==0) m=deg/2; //last coef to compute
				else m=(deg+1)/2;
				...
				{						
					...
					//remove factor 1-u
					if (lambda) for(j=0;j<ell;j++) coef[i][j]+=coef[i-1][j];
				...
				for(j=0;j<ell;j++) eqftmp1b[j]=BigInteger.valueOf(coef[deg-m][j]);
				...
				denomb=denomb.multiply(BigInteger.valueOf(p).pow(2*m-deg));
				...
				btmp=(long)Math.pow(p, 2*(m+1)-deg);
	
				for(j=m+1;j<=deg;j++)
				{
					coef[j]=multcmplx(eqfsign,conjugate(coef[deg-j],ell-1));
					for(i=0;i<ell-1;i++)
					{
